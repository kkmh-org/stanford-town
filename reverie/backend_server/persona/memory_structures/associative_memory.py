...
try:
  from persona.prompt_template.gpt_structure import _embeddings
except ImportError:
  _embeddings = None
...
class AssociativeMemory: 
  def __init__(self, f_saved): 
    ...
    self._init_vectorstore(f_saved)
  ...
  def save(self, out_json): 
    ...
    if self.vectorstore is not None:
      faiss_path = out_json + "/faiss_index"
      self.vectorstore.save_local(faiss_path)


  def _init_vectorstore(self, f_saved):
    """Initialize FAISS VectorStore from existing embeddings or saved index."""
    if _embeddings is None:
      return

    faiss_path = f_saved + "/faiss_index"
    if os.path.exists(faiss_path):
      try:
        self.vectorstore = FAISS.load_local(
            faiss_path, _embeddings,
    ...
    if self.embeddings:
      ...
      for key, vec in self.embeddings.items():
        ...
        metadatas.append({"embedding_key": key})
      if texts:
        self.vectorstore = FAISS.from_embeddings(
            list(zip(texts, emb_vectors)),
            _embeddings,
            metadatas=metadatas)
        return
  ...
  def _add_to_vectorstore(self, embedding_key, embedding_vec, node_id,
                          node_type, poignancy):
    """Write a new entry into the VectorStore (dual-write with self.embeddings)."""
    if self.vectorstore is None and _embeddings is not None:
      self.vectorstore = FAISS.from_embeddings(
          [(embedding_key, embedding_vec)],
          _embeddings,
          metadatas=[{"embedding_key": embedding_key,
                      "node_id": node_id,
                      "type": node_type,
                      "poignancy": poignancy}])
  ...
  def relevance_search(self, query_text, k=50):
    """
    Search the VectorStore for the k most relevant entries to query_text.
    Returns dict { embedding_key: similarity_score }.
    Uses FAISS similarity_search_with_score (L2 distance) and converts to
    a similarity value via 1/(1+distance).
    Falls back to None if VectorStore is unavailable.
    """
    if self.vectorstore is not None:
      try:
        results = self.vectorstore.similarity_search_with_score(
            query_text, k=k)
        return {doc.metadata["embedding_key"]: 1.0 / (1.0 + float(score))
  ...
  def add_event(self, created, expiration, s, p, o, 
    ...
    self._add_to_vectorstore(embedding_pair[0], embedding_pair[1],
                             node_id, "event", poignancy)
