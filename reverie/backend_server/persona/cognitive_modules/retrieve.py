...
def extract_relevance(persona, nodes, focal_pt): 
  ...
  Uses VectorStore when available (O(log n) via FAISS), falls back to
  brute-force cos_sim (O(n)) otherwise.
  """
  node_ids = {n.node_id for n in nodes}
  key_to_node_id = {n.embedding_key: n.node_id for n in nodes}

  vs_results = persona.a_mem.relevance_search(focal_pt, k=len(nodes) * 2)
  if vs_results is not None:
    relevance_out = dict()
    for emb_key, score in vs_results.items():
      if emb_key in key_to_node_id:
        relevance_out[key_to_node_id[emb_key]] = score
    for node in nodes:
